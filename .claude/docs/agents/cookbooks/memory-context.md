---
title: "Memory & context management with Claude Sonnet 4.6"
description: "Memory & context management with Claude Sonnet 4.6 Build AI agents with persistent memory using Claude's memory tool and context editing."
category: "agents"
---
...
Claude checked its memory (empty on first run)
Identified the bug: race condition - multiple threads modifying shared state (self.results and self.failed_urls) without synchronization
Stored the concurrency pattern in memory for future reference
...
Thinking blocks cleared - Old thinking from previous turns removed first
Tool results cleared - Old memory tool results removed when threshold exceeded
Memory files intact - Claude can still query learned patterns
Token usage managed - Saved tokens from both thinking and tool results
...
Recognizes similar pattern in async code (coroutines can interleave too)
Applies the solution immediately (no re-learning needed)
Explains with reference to stored knowledge
...
Store the pattern in /memories/concurrency_patterns/thread_safety.md
Apply this concurrency pattern to async code in Session 2
...
Session 1: Review Python code with a bug → Claude learns the pattern
Session 2: Review similar code (new conversation) → Claude applies the pattern
Session 3: Long review session → Context editing keeps it manageable
...
Content Sanitization: Filter dangerous patterns before storing
Memory Scope Isolation: Per-user/per-project isolation
Memory Auditing: Log and scan all memory operations
Prompt Engineering: Instruct Claude to ignore instructions in memory
...
Start with a single memory file for patterns (e.g., /memories/patterns.md)
Set context editing triggers at 30-40k tokens for production use
Implement per-project memory isolation to prevent cross-contamination
Other applications:
