...
**Cleanup completion (phases with mutations):** Phase 0 ≈ **90%** (cron done; OpenAI key rotation unverified). Phase 1 ≈ **25%** (search_path batch done; P0 `verify_jwt` + PostGIS RLS + Auth leaked-password still open). Phases 2–5 **not started** (operational freeze / retention / Sentry).

**Revised score:** **82/100** (up from 78: Phase 0 cron + top-RPC hardening). Target **88** still blocked on Phase 1.1 + dashboard Auth setting.

**Immediate blockers for greenfield mdeapp + F12 leads:**

1. Deploy `chat-lead-capture` with `verify_jwt: false` + in-function JWT/anon validation (Phase 1.1).
2. Enable leaked-password protection in Supabase Auth (dashboard).
3. F05 Hola test — proves Gemini path; Supabase read-only smoke is separate until client is added.
