...
| Agent path `writer.custom` no-op (UX-014) | 🔴 P0 | Restaurant cards never appear from agent |
...
**Disk note (2026-05-31):** `event` now mounts `RichCardResultsRegistrar` (line ~343). **Only restaurant/attraction** still use bare `GenericResults` without registrar or pin sync.
...
### P0 — Ship blockers
- **UX-014** — cards don't render on agent path
- **UX-022** — restaurant/attraction dup panel + broken map sync
- **UX-021** — accessibility violations
- **UX-027** — internal ticket strings in prod DOM

### P1 — UX consistency
- **UX-023** shell, **UX-024** hover, **UX-025** restaurant rich, **UX-028** fallback, **UX-030** tests

### P2 — Maintainability
- **UX-020** types (can land early), **UX-026** attraction, **UX-029** cleanup
...
flowchart TD
  ...
  RR -->|restaurant/attraction| G1[GenericResults ❌ no registrar ❌ no pinId]

  R1 --> CRC[CafeResultCard / RentalCard / EventCard]
  C1 --> CRC
  E1 --> EC[EventCard]
  G1 --> PRC[PlaceResultCard weak]

  G1 -.->|missing| REG[RichCardResultsRegistrar]
  G1 -.->|missing| SYNC[pinId onSelect selected]
...
1. **UX-022** + **UX-014** in parallel (pipeline + agent emit)
2. **UX-021** + **UX-027** quick wins same PR or immediately after
3. **UX-020** types land before or with **UX-023** shell
4. **UX-025** restaurant rich after shell
5. **UX-030** lock before marking card unification Done

Parent spec: [`../UX-010-unified-result-card-architecture.md`](../UX-010-unified-result-card-architecture.md)

Forensic verification: [`UX-TASKS-VERIFICATION-REPORT.md`](UX-TASKS-VERIFICATION-REPORT.md)
